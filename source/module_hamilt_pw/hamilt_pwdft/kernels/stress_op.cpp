        ...
        for (int i = 0; i < nkb; i++) {
            for (int ig = 0; ig < npw; ig++) {
                ...
                for (int ii = 0; ii < 3; ii++) {
                    qvec[ii]
                        = gcar[(ik * npwx + ig) * 3 + ii] + kvec_c[ik * 3 + ii];
                }
                auto pvkb1 = vkb1 + i * npwx;
                pvkb1[ig]
                    += static_cast<FPTYPE>(0.5) * qvec[ipol] * pvkb0j[ig]
                ...
                FPTYPE qvec_norm2
                    = qvec[0] * qvec[0] + qvec[1] * qvec[1] + qvec[2] * qvec[2];
                FPTYPE qm1 = qvec_norm2 > 1e-16 ? 1.0 / sqrt(qvec_norm2) : 0;
                pdbecp_noevc[ig] -= static_cast<FPTYPE>(2.0) * pvkb2[ig]
                                    * qvec[ipol] * qvec[jpol] * qm1 * tpiba;
            } // end ig
        }     // end nkb
    }
};
                    ...
                    for (int ia = 0; ia < atom_na[it]; ia++) {
                        for (int ip1 = 0; ip1 < Nprojs; ip1++) {
                            for (int ip2 = 0; ip2 < Nprojs; ip2++) {
                                ...
                                FPTYPE ps
                                    = deeq[((spin * deeq_2 + iat + ia) * deeq_3
                                            + ip1)
                                               * deeq_4
                                           + ip2]
                                      - ekb_now
                                            * qq_nt[it * deeq_3 * deeq_4
                                                    + ip1 * deeq_4 + ip2];
        ...
        for (int ix = 0; ix < 3; ix++) {
            for (int iy = 0; iy < ix + 1; iy++) {
                crosstaus[spin * nrxx * 6 + ipol * nrxx + ir]
                    += 2.0 * w1
                       * (gradwfc[ix * nrxx + ir].real()
                              * gradwfc[iy * nrxx + ir].real()
                          + gradwfc[ix * nrxx + ir].imag()
                                * gradwfc[iy * nrxx + ir].imag());
                ipol += 1;
            }
        }
    }
...
struct cal_vkb_op<FPTYPE, base_device::DEVICE_CPU> {
    void operator()(const base_device::DEVICE_CPU* ctx,
        ...
        for (int ih = 0; ih < nh; ih++) {
            ...
            for (int ig = 0; ig < npw; ig++) {
                vkb_ptr[ig]
                    = ylm_ptr[ig] * vq_ptr[ig] * sk_in[ig] * pref_in[ih];
            }
        }
    }
};
...
struct cal_vkb_deri_op<FPTYPE, base_device::DEVICE_CPU> {
    void operator()(const base_device::DEVICE_CPU* ctx,
        ...
        for (int ih = 0; ih < nh; ih++) {
            ...
            //  loop over all G-vectors
            const FPTYPE* ylm_deri_ptr1
                = ylms_deri_in + indexes[ih * 4 + 2] * npw;
            const FPTYPE* ylm_deri_ptr2
                = ylms_deri_in + indexes[ih * 4 + 3] * npw;
            const FPTYPE* vq_deri_ptr = vqs_deri_in + indexes[ih * 4 + 1] * npw;
            const FPTYPE* gkn = &gk_in[4 * npw];
            for (int ig = 0; ig < npw; ig++) {
                vkb_ptr[ig] -= (gk_in[ig * 3 + ipol] * ylm_deri_ptr2[ig]
                                + gk_in[ig * 3 + jpol] * ylm_deri_ptr1[ig])
            ...
            for (int ig = 0; ig < npw; ig++) {
                FPTYPE two = 2.0;
                vkb_ptr[ig] -= two * ylm_ptr[ig] * vq_deri_ptr[ig] * sk_in[ig]
                               * pref_in[ih] * gk_in[ig * 3 + ipol]
...
struct cal_vq_op<FPTYPE, base_device::DEVICE_CPU> {
    void operator()(const base_device::DEVICE_CPU* ctx,
        ...
        for (int nb = 0; nb < nbeta; nb++) {
            ...
            for (int ig = 0; ig < npw; ig++) {
                vq_ptr[ig] = _polynomial_interpolation<FPTYPE>(tab,
                                                               it,
                                                               nb,
                                                               tab_2,
                                                               tab_3,
...
struct cal_vq_deri_op<FPTYPE, base_device::DEVICE_CPU> {
    void operator()(const base_device::DEVICE_CPU* ctx,
        ...
        for (int nb = 0; nb < nbeta; nb++) {
            ...
            for (int ig = 0; ig < npw; ig++) {
                vq_ptr[ig]
                    = _polynomial_interpolation_nl<FPTYPE>(tab,
                                                           it,
                                                           nb,
                                                           tab_2,
                                                           tab_3,
...
template <typename FPTYPE>
void Simpson_Integral(const int mesh,
                      FPTYPE* func,
                      const FPTYPE* rab,
                      FPTYPE& asum) {
    assert(mesh & 1);
            ...
            for (int igl = igl0; igl < ngg; igl++) {
                ...
                for (int ir = 0; ir < mesh; ir++) {
                    if (type == 0) {
                        ...
                        aux[ir] = ir != 0 ? std::sin(gx_arr[igl] * r[ir])
                                                / (gx_arr[igl] * r[ir])
                                          : 1.0;
                ...
                if (type == 0)
                    drhocg[igl] = FOUR_PI / omega * rhocg1;
                else
                    drhocg[igl] = FOUR_PI * rhocg1 / omega;
            }
#ifdef _OPENMP
        }
#endif
...
//         FPTYPE* ylm_deri_in, FPTYPE** ylm_deri_ptr1s, FPTYPE**
//         ylm_deri_ptr2s, FPTYPE* vq_in, FPTYPE** vq_ptrs, FPTYPE* vq_deri_in,
//         FPTYPE** vq_deri_ptrs
//     ){
//         // int ih=0;
...
template <>
void pointer_array_malloc<base_device::DEVICE_CPU>::operator()(void** ptr,
                                                               const int n) {
    return;
}

template <>
void synchronize_ptrs<base_device::DEVICE_CPU>::operator()(void** ptr_out,
                                                           const void** ptr_in,
                                                           const int size) {
    return;
}
...
template struct cal_stress_mgga_op<std::complex<float>,
                                   base_device::DEVICE_CPU>;
template struct cal_stress_mgga_op<std::complex<double>,
                                   base_device::DEVICE_CPU>;
