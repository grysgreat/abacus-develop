...
void Forces<FPTYPE, Device>::cal_force(
    ...
    if (!GlobalV::use_paw) {
        ...
        Forces::cal_force_cc_new(forcecc, rho_basis, chr);
    } else {
        forcecc.zero_out();
    }
    ...
    if (vdw_solver != nullptr) {
        const std::vector<ModuleBase::Vector3<double>>& force_vdw_temp
            = vdw_solver->get_force();
    ...
    if (GlobalV::imp_sol) {
        ...
        GlobalC::solvent_model.cal_force_sol(GlobalC::ucell,
                                             rho_basis,
                                             forcesol);
        if (GlobalV::TEST_FORCE) {
            ModuleIO::print_force(GlobalV::ofs_running,
                                  GlobalC::ucell,
                                  "IMP_SOL      FORCE (Ry/Bohr)",
                                  forcesol);
        }
    }
    ...
    if (GlobalV::use_paw) {
        ...
        const std::tuple<double, double, ModuleBase::matrix> etxc_vtxc_v
            = XC_Functional::v_xc(rho_basis->nrxx,
                                  elec.charge,
                                  &GlobalC::ucell);
        v_xc = std::get<2>(etxc_vtxc_v);

        GlobalC::paw_cell.calculate_force(v_effective.c,
                                          v_xc.c,
                                          rhor,
                                          force_paw);
    ...
    for (int ipol = 0; ipol < 3; ipol++) {
        ...
        for (int it = 0; it < GlobalC::ucell.ntype; it++) {
            for (int ia = 0; ia < GlobalC::ucell.atoms[it].na; ia++) {
                force(iat, ipol) = forcelc(iat, ipol) + forceion(iat, ipol)
                                   + forcenl(iat, ipol) + forcecc(iat, ipol)
                                   + forcescc(iat, ipol);
                ...
                if (vdw_solver != nullptr) // linpz and jiyy added vdw force,
                                           // modified by zhengdy
                {
    ...
    if (GlobalV::GATE_FLAG || GlobalV::EFIELD_FLAG) {
        GlobalV::ofs_running << "Atomic forces are not shifted if gate_flag or "
                                "efield_flag == true!"
                             << std::endl;
    }
    ...
    GlobalV::ofs_running << std::setiosflags(std::ios::fixed)
                         << std::setprecision(6) << std::endl;
    /*if(GlobalV::TEST_FORCE)
    {
        ModuleIO::print_force(GlobalV::ofs_running, GlobalC::ucell,"LOCAL FORCE
    (Ry/Bohr)", forcelc); ModuleIO::print_force(GlobalV::ofs_running,
    GlobalC::ucell,"NONLOCAL FORCE (Ry/Bohr)", forcenl);
        ModuleIO::print_force(GlobalV::ofs_running, GlobalC::ucell,"NLCC FORCE
    (Ry/Bohr)", forcecc); ModuleIO::print_force(GlobalV::ofs_running,
    GlobalC::ucell,"ION      FORCE (Ry/Bohr)", forceion);
        ModuleIO::print_force(GlobalV::ofs_running, GlobalC::ucell,"SCC FORCE
    (Ry/Bohr)", forcescc); if(GlobalV::EFIELD)
    ModuleIO::print_force(GlobalV::ofs_running, GlobalC::ucell,"EFIELD   FORCE
    (Ry/Bohr)", force_e);
    }*/

    /*
        ModuleIO::print_force(GlobalV::ofs_running, GlobalC::ucell," TOTAL-FORCE
       (Ry/Bohr)", force);
       ...
       pengfei 2016-12-20
        {
            ...
            for(int iat=0; iat<this->nat; iat++)
            {
                ofs << "   " << force(iat,0)*ModuleBase::Ry_to_eV / 0.529177
                    ...
                    << "   " << force(iat,2)*ModuleBase::Ry_to_eV / 0.529177 <<
       std::endl;
            }
            ofs.close();
        }
    */
    ...
    if (GlobalV::TEST_FORCE) {
        ModuleIO::print_force(GlobalV::ofs_running,
                              GlobalC::ucell,
                              "LOCAL    FORCE (eV/Angstrom)",
                              forcelc,
                              false);
        ModuleIO::print_force(GlobalV::ofs_running,
                              GlobalC::ucell,
                              "NONLOCAL FORCE (eV/Angstrom)",
                              forcenl,
                              false);
        ModuleIO::print_force(GlobalV::ofs_running,
                              GlobalC::ucell,
                              "NLCC     FORCE (eV/Angstrom)",
                              forcecc,
                              false);
        ModuleIO::print_force(GlobalV::ofs_running,
                              GlobalC::ucell,
                              "ION      FORCE (eV/Angstrom)",
                              forceion,
                              false);
        ModuleIO::print_force(GlobalV::ofs_running,
                              GlobalC::ucell,
...
void Forces<FPTYPE, Device>::cal_force_loc(ModuleBase::matrix& forcelc,
    ...
    /*
        ...
        we can keep the blocked aux in L1 cache when iterating GlobalV::NSPIN
       loop performance will be better when number of atom is quite huge
    */
...
#pragma omp parallel for
    ...
    for (int irb = 0; irb < rho_basis->nrxx; irb += block_ir) {
        ...
        {                                           // is = 0
            for (int ir = irb; ir < ir_end; ++ir) { // initialize aux
                aux[ir] = std::complex<double>(chr->rho[0][ir], 0.0);
            }
        }
        if (GlobalV::NSPIN == 2) {
            for (int ir = irb; ir < ir_end; ++ir) { // accumulate aux
...
#pragma omp parallel for
    ...
    for (int iat = 0; iat < this->nat; ++iat) {
        ...
        for (int ig = 0; ig < rho_basis->npw; ig++) {
            const double phase
                = ModuleBase::TWO_PI
                  * (rho_basis->gcar[ig] * GlobalC::ucell.atoms[it].tau[ia]);
            double sinp, cosp;
            ModuleBase::libm::sincos(phase, &sinp, &cosp);
            const double factor
                = GlobalC::ppcell.vloc(it, rho_basis->ig2igg[ig])
...
void Forces<FPTYPE, Device>::cal_force_ew(ModuleBase::matrix& forceion,
    ...
    /*
        ...
        By blocking aux with block size 1024,
        we can keep the blocked aux in L1 cache when iterating
       GlobalC::ucell.ntype loop performance will be better when number of atom
       is quite huge
    */
...
#pragma omp parallel for
    ...
    for (int igb = 0; igb < rho_basis->npw; igb += block_ig) {
        ...
        for (int it = 0; it < GlobalC::ucell.ntype; it++) {
            ...
            for (int ig = igb; ig < ig_end; ++ig) { // accumulate aux
...
#ifdef USE_PAW
            charge
                += GlobalC::ucell.atoms[it].na * GlobalC::paw_cell.get_val(it);
#endif
        } else {
            charge
                += GlobalC::ucell.atoms[it].na
                   * GlobalC::ucell.atoms[it].ncpp.zv; // mohan modify 2007-11-7
        }
    }
    ...
    do {
        ...
        upperbound = 2.0 * charge * charge
                     * sqrt(2.0 * alpha / ModuleBase::TWO_PI)
                     * erfc(sqrt(GlobalC::ucell.tpiba2 * rho_basis->ggecut / 4.0
                                 / alpha));
...
#endif
    for (int ig = 0; ig < rho_basis->npw; ig++) {
        aux[ig] *= ModuleBase::libm::exp(-1.0 * rho_basis->gg[ig]
                                         * GlobalC::ucell.tpiba2 / alpha / 4.0)
...
#pragma omp parallel
    ...
    int thread_id = 0;
        ...
        /* Here is task distribution for multi-thread,
            0. atom will be iterated both in main nat loop and the loop in `if
           (rho_basis->ig_gge0 >= 0)`. To avoid syncing, we must calculate work
           range of each thread by our self
            1. Calculate the iat range [iat_beg, iat_end) by each thread
                a. when it is single thread stage, [iat_beg, iat_end) will be
           [0, nat)
        ...
        */
        ...
        int ig_gap
            = (rho_basis->ig_gge0 >= 0 && rho_basis->ig_gge0 < rho_basis->npw)
                  ? rho_basis->ig_gge0
                  : -1;
        ...
        // iterating atoms
        while (iat < iat_end) {
            if (it != last_it) { // calculate it_tact when it is changed
                double zv;
                if (GlobalV::use_paw) {
#ifdef USE_PAW
                ...
                it_fact = zv * ModuleBase::e2 * GlobalC::ucell.tpiba
            ...
            const auto ig_loop = [&](int ig_beg, int ig_end) {
                for (int ig = ig_beg; ig < ig_end; ig++) {
                    ...
                    const double arg
                        = ModuleBase::TWO_PI
                          * (gcar * GlobalC::ucell.atoms[it].tau[ia]);
                    double sinp, cosp;
        ...
        if (rho_basis->ig_gge0 >= 0) {
            ...
            ModuleBase::Vector3<double>* r
                = new ModuleBase::Vector3<double>[mxr];
            ...
            // iterating atoms.
            // do not need to sync threads because task range of each thread is
            // isolated
            while (iat1 < iat_end) {
                ...
                while (iat2 < this->nat) {
                    if (iat1 != iat2) {
                        ModuleBase::Vector3<double> d_tau
                            = GlobalC::ucell.atoms[T1].tau[I1]
                              - GlobalC::ucell.atoms[T2].tau[I2];
                        H_Ewald_pw::rgen(d_tau,
                                         rmax,
                                         irr,
                        ...
                        for (int n = 0; n < nrm; n++) {
                            ...
                            if (GlobalV::use_paw) {
#ifdef USE_PAW
                                factor = GlobalC::paw_cell.get_val(T1)
                                         * GlobalC::paw_cell.get_val(T2)
                                         * ModuleBase::e2 / (rr * rr)
                                         * (erfc(sqa * rr) / rr
                                            + sq8a_2pi
                                                  * ModuleBase::libm::exp(
                                ...
                                factor = GlobalC::ucell.atoms[T1].ncpp.zv
                                         * GlobalC::ucell.atoms[T2].ncpp.zv
                                         * ModuleBase::e2 / (rr * rr)
                                         * (erfc(sqa * rr) / rr
                                            + sq8a_2pi
                                                  * ModuleBase::libm::exp(
...
#else
        ModuleBase::WARNING_QUIT("cal_force_cc",
...
#pragma omp for nowait
                    ...
                    for (int ig = 0; ig < rho_basis->npw; ig++) {
                        ...
                        double sinp, cosp;
                        ModuleBase::libm::sincos(arg, &sinp, &cosp);
                        const std::complex<double> expiarg
                            = std::complex<double>(sinp, cosp);

                        auto ipol0 = GlobalC::ucell.tpiba * GlobalC::ucell.omega
                                     * rhocgigg * gv.x * psiv_conj * expiarg;
                        force[0] += ipol0.real();

                        auto ipol1 = GlobalC::ucell.tpiba * GlobalC::ucell.omega
                                     * rhocgigg * gv.y * psiv_conj * expiarg;
                        force[1] += ipol1.real();

                        auto ipol2 = GlobalC::ucell.tpiba * GlobalC::ucell.omega
                                     * rhocgigg * gv.z * psiv_conj * expiarg;
                        force[2] += ipol2.real();
                    }
#ifdef _OPENMP
...
#pragma omp atomic
    ...
    Parallel_Reduce::reduce_pool(
        forcecc.c,
        forcecc.nr * forcecc.nc); // qianrui fix a bug for kpar > 1
    ModuleBase::timer::tick("Forces", "cal_force_cc");
    return;
}
...
#pragma omp parallel for
        ...
        for (int ig = igg0; ig < rho_basis->ngg; ++ig) {
            ...
            const double gx
                = sqrt(rho_basis->gg_uniq[ig]) * GlobalC::ucell.tpiba;
...
#pragma omp parallel for reduction(+ : force0) reduction(+ : force1) reduction(+ : force2)
                    ...
                    // std::cout << " forcescc = " << forcescc(iat,0) << " " <<
                    // forcescc(iat,1) << " " << forcescc(iat,2) << std::endl;
                }
                iat++;
            }
        }
